...
import (
	...
	gaconfig "github.com/kudmo/CoolPA/internal/optimizer/provider/ga/config"
	"github.com/kudmo/CoolPA/internal/optimizer/provider/ga/constraints"
	"github.com/kudmo/CoolPA/internal/optimizer/provider/ga/engine"
	"github.com/kudmo/CoolPA/internal/optimizer/provider/ga/genome"
	slopredictor "github.com/kudmo/CoolPA/internal/optimizer/provider/slo_predictor"
...
func (ro *ReactionOptimizer) RunOptimization(ctx context.Context, services []string, mode optimizer.ScaleMode) (optimizer.OptimizedState, error) {
	...
	if err != nil {
		logger.Error("optimizer/provider", "ga engine run error", "error", err)
		return optimizer.OptimizedState{}, err
	}
	...
	state := optimizer.OptimizedState{Services: make([]optimizer.OptimizedServiceState, 0)}

	for _, service := range cand.Services {
		state.Services = append(state.Services,
			optimizer.OptimizedServiceState{
				...
				Reaction:    optimizer.ReactionType(service.Reaction),
			},
		)
	}
	return state, nil
...
func (ro *ReactionOptimizer) buildConstraints(
	...
	for _, svc := range services {
		replicas_count, _ := ro.metricsProvider.GetServiceReplicasCountValue(ctx, svc)
		if replicas_count == 0 {
			logger.Warn("optimizer/provider", "no pods found for service", "service", svc)
			continue
		}
...
func (ro *ReactionOptimizer) buildSeedGenome(
	...
	for _, svc := range services {
		...
		sign := 1.0
		if mode == optimizer.ScaleDownMode {
			sign = -1.0
		}
...
func (ro *ReactionOptimizer) logGenome(g *genome.ReactionGenome) {
	for _, gene := range g.Genes {
		logger.Debug("optimizer/provider", "best genome",
